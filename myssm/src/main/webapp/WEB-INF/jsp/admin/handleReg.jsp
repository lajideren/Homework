...
<%
    List<Apply> regs=(List)request.getAttribute("regs");
    int count=1;
%>
...
<div class="container" style="margin-top: 80px;width: 70%">
    ...
    <table id="myTable" class="table">
        ...
        <tbody>
            <%
                for(Apply apply:regs){
            %>
            <tr>
                <td><%=count++%></td>
                <td><%=apply.getEid()%></td>
                <td><%=apply.getCreateTime()%></td>
                <td><button data-toggle="modal" data-target="#myModal" onclick="showReg('<%=apply.getCreateTime()%>')" class="btn btn-success btn-sm">查看</button></td>
            </tr>
            <%
                }
            %>
...
<script type="application/javascript">
    function showReg(time) {
        $.ajax({
            type:"POST",
            url:"<%=ctx%>/admin/showReg",
            data:{time:time},
            dataType:"json",
            success:function (data) {
                ...
                $('#info').val(reg.info);
                $('#btn1').click(function() {
                    httpPost('<%=ctx%>/admin/approveReg', {time: reg.createTime, approved: 1});
                });
                $('#btn2').click(function() {
                    httpPost('<%=ctx%>/admin/approveReg', {time: reg.createTime, approved: -1});
                });
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 style="text-align: center;color:#0e77af">机构注册申请</h3>
            </div>
